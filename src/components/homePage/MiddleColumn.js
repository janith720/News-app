...
export default function MiddleColumn({ info, loading }) {
   ...
   const data = [
    ...
    return (
        <div>
            <h5 style={{background: '#E53935', color: 'white', padding: '8px',transform: 'skew(-5deg)'}}>International News</h5>
                ...
                ? data.map((val,key) => {
                    return (
                            ...
                            <Grid key={key} item xs={12} sm={6} lg={6}>
                              <Link style={{textDecoration: 'none'}} to={`/internationalDetails/${key}`}>
                                <NewsTiles  key={key} newsImage={val.image} newsTitle={val.caption} date={val.date} />
                              </Link>
                            </Grid>
